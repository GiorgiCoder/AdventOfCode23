...
namespace AdventOfCode.AdventOfCode2023.Day07
{
    internal class Part2
    {
        ...
        static int compareHands(string _hand1, string _hand2) // returns 1 if hand1 > hand2 and -1 if hand1 < hand2
        {
            ...
            int numOfJ1 = 0, numOfJ2 = 0;

            if (charsOfHand1.ContainsKey('J'))
            {
                numOfJ1 = charsOfHand1['J'];
                if (numOfJ1 < 5)
                {
                    charsOfHand1.Remove('J');
                    char maxKey = charsOfHand1.Aggregate((x, y) => x.Value > y.Value ? x : y).Key;
                    charsOfHand1[maxKey] += numOfJ1;
                }
            }
            if (charsOfHand2.ContainsKey('J'))
            {
                numOfJ2 = charsOfHand2['J'];
                if (numOfJ2 < 5)
                {
                    charsOfHand2.Remove('J');
                    char maxKey = charsOfHand2.Aggregate((x, y) => x.Value > y.Value ? x : y).Key;
                    charsOfHand2[maxKey] += numOfJ2;
                }
            }
